    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(url, proxy=get_proxy(), timeout=40) as r:
                if r.status != 200:
                    await post_error(f"SAM {r.status}")
                    return []
                try:
                    data = await r.json()
                except Exception:
                    await post_error("SAM: invalid JSON response")
                    return []
                return data.get("opportunitiesData", [])
    except Exception as e:
        await post_error(f"SAM: {e}")
        return []
...
async def loop_once():
    ...
    for i in items:
        f = {
            "Opportunity Name": i.get("title"),
            "Agency": i.get("organizationName"),
            "Total Value": i.get("awardAmount"),
            "NAICS Code": ",".join(i.get("naicsCodes", [])),
            "Submission Deadline": i.get("responseDeadLine"),
            "Link": i.get("uiLink"),
        }
        if f["Opportunity Name"] and f["Link"]:
